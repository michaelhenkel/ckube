...
func runQemu(args []string) {
	...
	deviceFlags := utils.MultipleFlag{}
	...
	*accel = utils.GetStringValue("LINUXKIT_QEMU_ACCEL", *accel, "")
	...
	if !*kernelBoot && !*isoBoot {
		...
		d := utils.Disks{utils.DiskConfig{Path: diskPath}}
...
func buildQemuForwardings(publishFlags utils.MultipleFlag) (string, error) {
	if len(publishFlags) == 0 {
		return "", nil
	}
...
func buildDockerForwardings(publishedPorts []string) ([]string, error) {
	...
	for _, port := range publishedPorts {
		s, err := utils.NewPublishedPort(port)
		if err != nil {
			return nil, err
		}
