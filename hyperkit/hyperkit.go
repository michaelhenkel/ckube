...
func (h *HyperKit) execute() (*exec.Cmd, error) {
	...
	if h.Console == ConsoleStdio {
		...
		stdout, err := os.Create(filepath.Join(h.StateDir, "hyperkit.stdout"))
		if err != nil {
			fmt.Printf("error creating hyperkit stdout: %v", err)
			return nil, err
		}
		defer stdout.Close()

		stderr, err := os.Create(filepath.Join(h.StateDir, "hyperkit.stderr"))
		if err != nil {
			fmt.Printf("error creating hyperkit stderr: %v", err)
			return nil, err
		}
		defer stderr.Close()
		cmd.Stdout = stdout
